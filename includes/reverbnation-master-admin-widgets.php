<?php
function menu_single_reverbnation_admin_widgets(){
	if ( is_admin() )
	add_submenu_page( 'reverbnation-master', 'Widgets', 'Widgets', 'manage_options', 'reverbnation-master-admin-widgets', 'reverbnation_master_admin_widgets' );
}

function reverbnation_master_admin_widgets(){
$plugin_master_name = constant('REVERBNATION_MASTER_NAME');
?>
<div class="wrap">
<h1>Widgets</h1>
<?php
if(!class_exists('reverbnation_master_admin_widgets_table')){
	require_once( dirname( __FILE__ ) . '/reverbnation-master-admin-widgets-table.php');
}
//Prepare Table of elements
$wp_list_table = new reverbnation_master_admin_widgets_table();
//Table of elements
...
if( is_multisite() ) {
add_action( 'admin_menu', 'menu_single_reverbnation_admin_widgets' );
}
else {
add_action( 'admin_menu', 'menu_single_reverbnation_admin_widgets' );
}
